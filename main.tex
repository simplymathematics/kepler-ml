...
\author[2,1]{\fnm{Mohammad} \sur{Reza Saleh Sedghpour}}\email{msaleh@cs.umu.se}


\author[1,2]{\fnm{Erik} \sur{Elmroth}}\email{elmroth@cs.umu.se}

% \pagestyle{empty}
\author[1]{\fnm{Tommy} \sur{L\"{o}fstedt}}\email{tommy@cs.umu.se}

\affil*[1]{\orgdiv{Department of Computing Science}, \orgname{Ume\aa{}~University},
\city{Ume\aa}, \postcode{901 87}, \country{Sweden}}
...
Additionally, testing model changes likely means deploying the models to (\textit{e.g.} a car or a medical imaging device), or a drone to see how it affects performance, making un-tested changes a public problem that reduces development speed, increases cost of development, and makes it difficult (if not impossible) to parse cause from effect.
...
This provides a way to evaluate the model and optimise it in a single step, while simultaneously allowing us to model the effect of model parameters on training time, prediction time, and accuracy. 
Using this technique, we demonstrate that newer, more-powerful hardware does decrease the training time, but with a monetary and power cost that far outpaces the marginal gains in accuracy.
}
...
All authors consent for this manuscript to be published under the terms specified by the \textit{Journal of Cloud Computing}.
...
% The authors have no conflict of interest regarding the content of this work.
% \subsection{Availability of Data and Materials}
% The source code for conducting the experiments is available in the \texttt{deckard} git repository located \href{https://github.com/simplymathematics/deckard/tree/main/examples/power}{here}.
% The pretrained \texttt{pytorch} version of ResNet18 was used to conduct the experiments. Requirements and dependencies are listed within the repository, which is distributed under the GNU Public License version 3. The compiled data and the Python source code for this document are available \href{https://github.com/simplymathematics/kepler-ml}{here}. The source code has been tested on Windows and various Linux distributions and should work on MacOS, but has not been tested there.
% \subsection{Author's Contributions}
% CM wrote most of the manuscript, except the portions focused on cloud architectures, which were authored by MRSS. The math sections were clarified and edited by TL. CM prepared all figures. Source code was mostly written by CM, except the cloud architecture components, which were written by MRSS. EE provided guidance on narrative, pedagogy, and clarity. All authors reviewed the manuscript.
