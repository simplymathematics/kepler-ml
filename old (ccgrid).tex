...
%  Moved to main.tex for cloud conference
...
% Additionally, testing model changes likely means deploying the models to a car, medical imaging device, or drone to see how it affects performance, making un-tested changes a public problem that reduces development speed, increases cost of development, and makes it difficult (if not impossible) to parse cause from effect. We address the problem of neural network validation and verification. More specifically, we address the problem of continuous integration and deployment for safety-critical models.
% For neural networks in particular, the relationship between model learning rate, batch size, training time, convergence time, and deployment cost is highly complex, meaning that each software change would need to be evaluated against new real-world deployment samples which can be prohibitively expensive or time-consuming to collect. To address this, we propose using accelerated failure time models to measure the effect of hardware choice, batch size, number of epochs, and learning rate by using adversarial attacks to induce failures on a reference model architecture. We evaluate several GPU types and use the Tree Parzen Estimator to maximize model robustness and minimize model run-time simultaneously. This provides a way to evaluate the model and optimize it in a single step, while simultaneously allowing us to model the effect of model parameters on training time, prediction time, and accuracy. Using this technique, we demonstrate that newer, more-powerful hardware does decrease the training time, but with a monetary and power cost that far outpaces the marginal gains in accuracy.
% \end{abstract}
...
%     \item demonstrates a scalable and effective method that allows us to tune a model while simultaneously estimating the asymptotic effect of various hyperparameters,
%     \item and measures the power and monetary cost of deploying a model across different hardware architectures to model the trade-offs between deployment hardware and robustness.
%     \item We show that, despite a decrease in training time, that faster hardware, more precise hardware, and/or more power-hungry hardware do not lead to more adversarial robust models.
...
% To address these challenges, containerization and orchestration technologies have gained prominence in managing complex ML pipelines. Kubernetes, a widely adopted container orchestration platform, provides a scalable and flexible environment for deploying, scaling, and managing microservice-based applications. Its ability to handle heterogeneous hardware architectures and workload distribution across clusters makes it an ideal choice for managing the complex computational requirements of ML pipelines.

% % Using this microservice architecture, it's possible explore the robustness of the pipeline under adversarial attacks and examine the impact of computational cost on model performance in both benign and adversarial contexts across heterogeneous hardware to minimize the deployment cost in the cloud.
...
% In this paper, we present a comprehensive study that leverages Kubernetes as the underlying infrastructure for managing a complex ML pipeline across a variety of GPU architectures. We then investigate the robustness of the pipeline against adversarial attacks and explore the relationship between computational cost, model loss, and  prediction accuracy in both benign and adversarial contexts. Our experiments and analysis provide valuable insights into the trade-offs and considerations when deploying ML models under varying computational constraints and adversarial conditions.
...
% \cm{Moved Accuracy/ Failure Rate to this section}
% \section{AFR models for Robustness Verification during Training}
% \label{afr}
% Accelerated failure rate models are statistical models used to analyze multivariate effects on the observed failure rate to predict the time-to-failure across a wide variety of circumstances~\cite{aft_models,kleinbaum1996survival}. 
...
%  To encompass the cost of a particular model or attack, we can think of the failure rate as a function of time (\textit{e.g.}, training time, inference time, attack generation time, \textit{etc.}) and some covariates (\textit{e.g.}, noise distance, model size, hardware resources, $\theta$) such that,
% \[
%     h_{ \theta}(t) :=  \frac{P(\textrm{False~Classification})}{\Delta t} t
% \]
% where $P(\textrm{False~Classification})$ is the probability of a false classification and $\Delta t$ is a time interval. \cm{Note that this is equivalent to 1 - accuracy when $\Delta t = t$.} To get a more precise and reliable estimate of a model's failure characteristics, we can use accelerated failure time models that model the \textit{survival time}, $S(t)$, as a function of the time, $t$,  and some set of model parameters, $\theta$ such that,
% $$
% S_{\theta}(t) = \exp\left(-\int_0^t h_{\theta}(u) \, du\right)
% $$
% We can use the survival function to compute the expected survival time or average time-to-failure:
% \[
...
% \subsubsection{Attack Generation Time}
...
% Correspondingly, we can talk about the expected survival time under optimistic, $S_{ben.}$ as well as under adverse (accelerated failure rate distribution), $S_{adv.}$. If we assume that the distribution of failures is uniform with respect to latency \footnote{\cm{While this is not necessarily the case, neural networks generally assume independent and identically distributed data~\cite{ma2022state}, but we admit that tracking the per-sample failure rate would be a good way to detect under-sampled regions or otherwise likely-to-fail samples. Furthermore, since the modelling distributions include shape and scale parameters for each covariate as well as the model as a whole, whether or not we divide by the number of samples is irrelevant since these shape and scale parameters will absorb this value anyway. We include this normalization so that we can compare the benign and adversarial prediction times which are calculated across 10 thousand and 100 samples respectively. That is, we merely scaled the y-axis on Figs~\ref{fig:time}~and~\ref{fig:cost} without changing the behaviour of $S(t)$ for the sake of comparison.}}
% $$
...
% \subsection{Time}
% Fig.~\ref{fig:time} reveals the training time proportional to the GPU bandwidth numbers in Tab.~\ref{tab:hardware}. Slower hardware is only a few milliseconds slower, so this is unlikely to be noticeable to an end-user during inference. This should be no surprise since the less-capable hardware is meant to be used exclusively for inference. Additionally, we see that the attack time is colinear with both the training and prediction times across hardware and datasets-- an expected result since this will be driven by the number of samples and the inference time (see: Eq.~\ref{attack_time}). 
% \begin{figure}[h!]
%     \centering
%     \includegraphics[width=.56\textwidth]{plots/combined/time.pdf}
...
% \begin{figure}
%     \centering
%     \includegraphics[width=.5\textwidth]{plots/combined/power.pdf}
%     \caption{Training, Prediction, and Attack Power Consumption Across Datasets and Hardware}
%     \label{fig:power}
% \end{figure}

% \subsection{Partial Effects of Covariates}
% Figure~\ref{fig:partial_effect_train_time} shows that training time, by itself, does little to influence the model's survival time. However, as we see in Fig.~\ref{fig:partial_effect_attack_eps}, that increase the attack distance by an order of magnitude reduces the survival time marginally. Furthermore, Table~\ref{tab:Combined} shows that the log-logistic and log-normal distributions have similar predictive power, but we used the log-logistic model to render the plots due to its marginally improved AIC. 



% \begin{figure}
%     \centering
%     \includegraphics[width=.5\textwidth]{plots/combined/cost.pdf}
...
%   \centering
%   % include first image
%   \includegraphics[width=.8\linewidth]{plots/combined/log_logistic_train_time_partial_effect.pdf}  
%   \caption{This depicts the partial effect of the training time on the survival time across all datasets and models according to the distribution $S(t)$ as outlined in Section~\ref{survival_time}.}
...
%   \includegraphics[width=.8\linewidth]{plots/combined/log_logistic_accuracy_partial_effect.pdf} 
%   \caption{This depicts the partial effect of the accuracy on the survival time across all datasets and models according to the distribution $S(t)$ as outlined in Section~\ref{survival_time}.}
%   \label{fig:partial_effect_accuracy}
% \end{figure}
...
%   \centering
%   % include second image
%   \includegraphics[width=.8\linewidth]{plots/combined/log_logistic_adv_failure_rate_partial_effect.pdf} 
%   \caption{This depicts the partial effect of the adversarial failure rate on the survival time across all datasets and models according to the distribution $S(t)$ as outlined in Section~\ref{survival_time}.}
%   \label{fig:partial_effect_adv_failure_rate}
% \end{figure}

% \begin{figure}
%   \centering
%   % include second image
%   \includegraphics[width=.8\linewidth]{plots/combined/log_logistic_attack_eps_partial_effect.pdf}  
%   \caption{This depicts the partial effct of the attack distance on the survival time across all datasets and models according to the distribution $S(t)$ as outlined in Section~\ref{survival_time}.}
...
% \section{Conclusion}
% In this work, we introduced survival analysis to machine learning, developed a method for quickly and efficiently test model parameter choices and to evaluate them in the presence of adversarial noise, allowing us the model the Pareto front as a function of tested model parameters. Using this technique, we showed that despite reduced run-times with newer and/or larger hardware, that adversarial robustness is not significantly effected and that the specific choice of attack parameters is far more important than model training time. Furthermore, we demonstrate the un-advertised training capability of the Nvidia L4 GPU which can get substantially similar results when compared to more expensive and power hungry hardware. However, the marginal run-time is significant, even if it's cost-effective.
